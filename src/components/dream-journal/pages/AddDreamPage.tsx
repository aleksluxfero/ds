...
import { getDreamById, getAllUniqueTags } from '@/services/api';
...
const AddDreamPage: React.FC = () => {
  ...
  const { addDream, updateDream } = useDreams();
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (isEditMode && dreamId) {
        await updateDream(dreamId, dreamPayload);
      } else {
        await addDream(dreamPayload);
        localStorage.removeItem(DRAFT_KEY);
      }
      router.back();
    } catch (err) {
  ...
  }, [isSaving, title, content, tags, type, date, isDateUnknown, isEditMode, dreamId, router, addDream, updateDream]);
