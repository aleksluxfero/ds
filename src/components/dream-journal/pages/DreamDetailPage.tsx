...
import { useDreamQuery, useDeleteDreamMutation } from '@/hooks/useDreamsQuery';
...
const DreamDetailPage: React.FC = () => {
  ...
  const { data: dream, isLoading } = useDreamQuery(initData || '', dreamId || 0);
  ...
  const handleConfirmDelete = async () => {
    if (dream) {
      try {
        await deleteMutation.mutateAsync(dream.id);
        router.push('/');
      } catch (error) {
